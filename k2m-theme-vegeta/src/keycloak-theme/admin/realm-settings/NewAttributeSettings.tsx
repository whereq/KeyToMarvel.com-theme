...
export default function NewAttributeSettings() {
  ...
  const save = async ({
    ...
    // K2M-PATCH: The translation pre-loading (useFetch effect) races against the
    // attribute data fetch, so translationsData.translations may be empty even when
    // realm_localizations has the correct values. The Keycloak REST API does not
    // enforce this client-side guard server-side, so it is safe to skip it here.
    // Translations are still persisted by saveTranslations() below when loaded.
    // See: docs/keycloak-user-profile-i18n-translation-error.md
